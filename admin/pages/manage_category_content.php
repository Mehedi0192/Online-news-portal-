<?php
    ...
    if(isset($_GET['status'])) {
        ...
        if($_GET['status'] == 'unpublished') {
            $meassage=$obj_sup_admin->unpublished_category_by_id($category_id); 
        }
        else if($_GET['status'] == 'published') {
            $meassage=$obj_sup_admin->published_category_by_id($category_id); 
        }
        else if($_GET['status'] == 'delete') {
            $meassage=$obj_sup_admin->delete_category_by_id($category_id); 
        }
    }



    $query_result=$obj_sup_admin->select_all_category_info();
?>
<div class="row-fluid sortable">		
    <div class="box span12">
        <div class="box-header" data-original-title>
            <h2><i class="halflings-icon box"></i><span class="break"></span>All Categories</h2>
            <div class="box-icon">
